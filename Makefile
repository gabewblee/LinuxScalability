...
.PHONY: all clean download initramfs lebench plot run run-all setup

all: setup lebench initramfs run-all plot
...
run: initramfs download
	@bash $(SCRIPTS)/benchmark.sh $(VERSION)
